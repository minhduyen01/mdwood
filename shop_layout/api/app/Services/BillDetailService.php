...
class BillDetailService implements BillDetailServiceInterface
{
    ...
    {
        return $this->billDetail->select(DB::raw('DATE(bill_detail.created_at) as date'), DB::raw('SUM(unit_price * quantity * (100 - sale) / 100) as sale'))
    ...
    {
        $now = now();
        return $this->billDetail->select(DB::raw('MONTH(bill_detail.created_at) as month'), DB::raw('SUM(unit_price * quantity * (100 - sale) / 100) as sale'))
        ->join('bills', 'bills.id', '=', 'bill_detail.bill_id')
        ->where('bills.status', 1)
