  ...
  <div class="content">
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <h4 class="m-0" style="float: left">Thêm nhà cung cấp</h4>
      </div>
      <div>
        <el-form ref="formItemName" :model="supplier" label-width="140px" :rules="rules" label-position="top">
          ...
          <el-col :span="23">
            <el-form-item style="float: right">
              <el-button type="primary" @click="createSupp()">Thêm mới</el-button>
            </el-form-item>
          </el-col>
        </el-form>
      </div>
  ...
  methods: {
    createSupp() {
      this.$refs.formItemName.validate((valid) => {
        if(valid) {
          this.$axios.post('/admin/supplier', this.supplier).then((res) => {
            Swal.fire({
              icon: 'success',
